#!/bin/bash
# =============================================================================
# Frozen-generator ablation: the bug generator injects bugs into correct code
# but is never trained; only the fixer learns. Isolates how much of anchored
# self-play's gain comes from *training* the generator rather than merely
# having one.
#
# Contrast with train_fixer_flow.sh, where a frozen synthesizer writes code
# from scratch and the fixer repairs its failures.
#
# Usage:
...
#   GENERATOR_MODEL, GENERATOR_BASE_URL.
# =============================================================================
set -euo pipefail
...
NUM_GPUS="${NUM_GPUS:-8}"
RUN_NAME="${RUN_NAME:-frozen-generator-fixer}"
...
# Frozen bug generator. Point at any OpenAI-compatible endpoint; leave
# GENERATOR_MODEL empty to reuse the fixer's own model.
GENERATOR_MODEL="${GENERATOR_MODEL:-openai/gpt-oss-20b}"
GENERATOR_BASE_URL="${GENERATOR_BASE_URL:-http://localhost:32000/v1}"
...
python3 -m examples.asp.train_frozen_generator_fixer_flow \
    ...
    +rllm.workflow.workflow_args.generator_model="$GENERATOR_MODEL" \
    +rllm.workflow.workflow_args.generator_base_url="$GENERATOR_BASE_URL" \
    +rllm.workflow.workflow_args.generator_temperature=$GENERATOR_TEMPERATURE \
    +rllm.workflow.workflow_args.generator_top_p=$GENERATOR_TOP_P \
    +rllm.workflow.workflow_args.fixer_reward_pm1=$REWARD_PM1 \
