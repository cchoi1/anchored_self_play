    ...
    def transform_results_for_verl(self, episodes: list[Episode], task_ids: np.ndarray) -> "DataProto":
        ...
        def map_role(name: str) -> str:
            if name == "solver":
                ...
                role_name = "cm"
            elif name == "bug_generator":
                role_name = "generator"
            elif name.startswith("bug_fixer"):
                role_name = "solver"
            else:
                role_name = name
            return role_name
        ...
        for i, episode in enumerate(episodes):
            ...
            enable_bug_uid = (info.get("workflow") == "generator_fixer")
            ep_bug_id = info.get("bug_id") if enable_bug_uid else None
            for trajectory in episode.trajectories:
                name = trajectory.name
                trajectory_id = f"{episode_id}_{name}"

                if len(trajectory.steps) == 0:
                    logger.info(f"Trajectory {trajectory_id} has no steps, skipping")
                    continue
                
                role_name = map_role(name)
                ...
                if not self.config.rllm.stepwise_advantage.enable:
                    ...
                    traj_rewards.append(float(trajectory.reward))
                    # IDs: must append exactly one id for this row
                    trajectory_ids.append(trajectory_id)
                    if ep_bug_id is not None and name.startswith("bug_fixer"):
                        step_ids.append(f"{trajectory_id}_bug{ep_bug_id}_step0")
                    else:
                        step_ids.append(f"{trajectory_id}_step0")
                    step_nums.append(1)
                    is_last_step.append(True)
                    roles.append(role_name)
                    total_rows_this_episode += 1
                    # =====================
                    ...
                    for step_idx, step in enumerate(trajectory.steps):
                        ...
                        trajectory_ids.append(trajectory_id)
                        if ep_bug_id is not None and name.startswith("bug_fixer"):
                            step_ids.append(f"{trajectory_id}_bug{ep_bug_id}_step{step_idx}")
                        else:
                            step_ids.append(f"{trajectory_id}_step{step_idx}")
                        step_nums.append(n_steps)
                        
                        is_last_step.append(step_idx == (n_steps - 1))
                        roles.append(role_name)
                        total_rows_this_episode += 1
                        # ================================================

            episode_ids.extend([episode.id] * total_rows_this_episode)
            is_correct.extend([episode.is_correct] * total_rows_this_episode)
            termination_reasons.extend([episode.termination_reason if episode.termination_reason is not None else TerminationReason.UNKNOWN] * total_rows_this_episode)
            metrics.extend([episode.metrics] * total_rows_this_episode)
            repeat_counts.append(total_rows_this_episode)
        ...
        return DataProto.from_dict(
            ...
            non_tensors={
                ...
                "role": np.array(roles, dtype=object),  # NEW
            },
            meta_info={
...
#                 # Map trajectory.name -> role string
#                 # Canonical roles: "generator", "solver", "cm" (context_manager)
#                 if name == "solver":
#                     role_name = "solver"
#                 elif name == "context_manager":
#                     role_name = "cm"
...
#                     role_name = "generator"
#                 elif name.startswith("bug_fixer"):
#                     # bug_fixer, bug_fixer_0, bug_fixer_1, ... all map to "solver"
#                     role_name = "solver"
#                 elif name == "code_generator":
#                     # codegen eval (validation only, typically no gradient)
#                     role_name = "solver"
#                 else:
#                     role_name = name  # fallback if you add more agents later
#                 roles.extend([role_name] * n_steps)  # one role per step
